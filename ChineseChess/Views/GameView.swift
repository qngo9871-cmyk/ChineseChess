...
/// Main game view — draws the Xiangqi board with Canvas and handles interaction.
struct GameView: View {
    ...
    private var turnIndicator: some View {
        HStack(spacing: 8) {
            Circle()
                .fill(game.currentTurn == .red ? Color.red : Color.black)
    ...
    private func boardCanvas(cellSize: CGFloat, origin: CGPoint) -> some View {
        Canvas { context, _ in
            ...
            // Vertical lines — inner columns break at the river
            for col in 0..<Board.columns {
                let x = ox + CGFloat(col) * cellSize
                var path = Path()
                if col == 0 || col == Board.columns - 1 {
                    path.move(to: CGPoint(x: x, y: oy))
                    path.addLine(to: CGPoint(x: x, y: oy + CGFloat(Board.rows - 1) * cellSize))
                } else {
                    path.move(to: CGPoint(x: x, y: oy))
                    path.addLine(to: CGPoint(x: x, y: oy + 4 * cellSize))
                    path.move(to: CGPoint(x: x, y: oy + 5 * cellSize))
                    path.addLine(to: CGPoint(x: x, y: oy + CGFloat(Board.rows - 1) * cellSize))
                }
                context.stroke(path, with: .color(lineColor), lineWidth: 1.2)
            }
            ...
            // River text
            let riverY = oy + 4.5 * cellSize
            let riverRect = CGRect(x: ox + cellSize, y: riverY - cellSize * 0.35,
                                   width: CGFloat(Board.columns - 3) * cellSize,
                                   height: cellSize * 0.7)
            context.draw(
                Text("楚 河          漢 界")
            ...
            // Highlight selected piece with blue ring
            if let sel = game.selectedPiece {
                ...
                context.stroke(Ellipse().path(in: rect), with: .color(.blue), lineWidth: 2.5)
            }
        }
    }
    ...
    private func piecesOverlay(cellSize: CGFloat, origin: CGPoint) -> some View {
        ForEach(game.board.pieces) { piece in
            ...
            ZStack {
                ...
                Circle()
                    .strokeBorder(piece.color == .red ? Color.red : Color.black, lineWidth: 2)
    ...
    private func handleTap(at location: CGPoint, cellSize: CGFloat, origin: CGPoint) {
        ...
        // After a human move, trigger AI if applicable.
